...
static void StringAppendVT(StringType* dst,
  ...
  va_list ap_copy = ap;
  int array_size = sizeof(stack_buf) / sizeof(StringType::value_type);

  int result = vsnprintfT(stack_buf, array_size, format, ap_copy);
  va_end(ap_copy);

  if (result >= 0 && result < static_cast<int>(array_size)) {
    // It fit.
