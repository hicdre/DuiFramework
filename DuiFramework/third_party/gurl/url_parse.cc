...
bool ExtractQueryKeyValue(const wchar_t* url,
                          Component* query,
                          Component* key,
                          Component* value) {
  return DoExtractQueryKeyValue(url, query, key, value);
...
void ParseAuthority(const wchar_t* spec,
                    const Component& auth,
                    Component* username,
                    Component* password,
                    Component* hostname,
...
void ParseFileSystemURL(const wchar_t* url, int url_len, Parsed* parsed) {
  DoParseFileSystemURL(url, url_len, parsed);
}
...
void ParseMailtoURL(const wchar_t* url, int url_len, Parsed* parsed) {
  DoParseMailtoURL(url, url_len, parsed);
}
